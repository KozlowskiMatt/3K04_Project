...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    if (terminatingmodel)
      break;
    board_parameters_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(board_parameters_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    if (rtmStepTask(board_parameters_M, 1)
        ) {
      mw_osSignalEventRelease(&subrateTaskSem[0]);
    }

    board_parameters_step(0);

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(board_parameters_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(board_parameters_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(board_parameters_M, 0);

  /* Initialize model */
  board_parameters_initialize();
